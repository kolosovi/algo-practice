...
public:
    std::vector<std::vector<std::string>> accountsMerge(std::vector<std::vector<std::string>> &accounts) {
        ...
        for (int account_i = 0; account_i < accounts.size(); ++account_i) {
            ...
            for (; unprocessed_emails_start < account.size(); unprocessed_emails_start++) {
                const auto &indexes = email_to_indexes[account[unprocessed_emails_start]];
                for (auto index : indexes) {
                    ...
                    for (int email_i = 1; email_i < other_account.size(); ++email_i) {
                        auto email_it = std::find(account.begin(), account.end(), other_account[email_i]);
