...
class Solution {
 public:
  ...
  int maxHalfPathSum(TreeNode *node, int &maxPathSum) {
    ...
    auto result = std::max({node->val, node->val + maxLeftHalfPathSum,
                            node->val + maxRightHalfPathSum});
    maxPathSum =
        std::max({maxPathSum, result,
                  node->val + maxLeftHalfPathSum + maxRightHalfPathSum});
    return result;
  }
};
