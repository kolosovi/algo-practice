...
class Visitor {
 public:
  Visitor(std::unordered_map<std::string, std::vector<std::string>> &children,
          std::unordered_map<std::vector<std::string>, double, EquationHasher,
                             EquationEqualer> &divs,
          std::string target)
      : children_(children), divs_(divs), target_(target) {}
