...
class Solution {
 public:
  ...
  bool flipEquivRec(TreeNode *cur1, TreeNode *cur2) {
    ...
    auto *second_child_1 =
        cur1->left != first_child_1 ? cur1->left : cur1->right;
    auto *first_child_2 = cur2->left != nullptr ? cur2->left : cur2->right;
    auto *second_child_2 =
        cur2->left != first_child_2 ? cur2->left : cur2->right;
    ...
    // still no match — exit
    if (first_child_1->val != first_child_2->val ||
        second_child_1->val != second_child_2->val) {
      return false;
    }
    // there's a match, check children recursively
    return flipEquivRec(first_child_1, first_child_2) &&
           flipEquivRec(second_child_1, second_child_2);
  }
};
