 ...
 public:
  double mincostToHireWorkers(std::vector<int>& quality, std::vector<int>& wage,
    ...
    std::sort(workers.begin(), workers.end(),
              [&quality, &wage](int left, int right) {
                auto left_coeff = wage[left] * quality[right],
                     right_coeff = wage[right] * quality[left];
    ...
    for (auto worker : workers) {
      if (past.size() == k - 1) {
        double cost = static_cast<double>(wage[worker]) +
                      (static_cast<double>(past_quality) *
                       static_cast<double>(wage[worker])) /
                          static_cast<double>(quality[worker]);
