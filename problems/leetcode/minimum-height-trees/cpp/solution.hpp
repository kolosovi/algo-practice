...
private:
    void dfs1(int node, const std::vector<std::vector<int>> &children, int parent, int depth, int &max_node, int &diameter) {
        if (depth >= diameter) {
            diameter = depth;
            max_node = node;
        }
    ...
    bool dfs2(int node, const std::vector<std::vector<int>> &children, std::vector<int> &result, int parent, int depth, int target_depth, int diameter) {
        if (depth == diameter) {
            if (depth == target_depth || (diameter % 2 == 1 && depth == target_depth - 1)) {
        ...
        for (auto child : children[node]) {
            ...
            if (dfs2(child, children, result, node, depth+1, target_depth, diameter)) {
                if (depth == target_depth || (diameter % 2 == 1 && depth == target_depth - 1)) {
                    result.push_back(node);
                }
                return true;
