...
public:
    ...
    int calculate(std::vector<std::string> &strs, std::vector<std::vector<std::vector<int>>> &memo, int i, int m, int n) {
        if (i == strs.size()) {
            return 0;
        }
        if (memo[i][m][n] != -1) {
        ...
        if (zero_count <= m && one_count <= n) {
            max = std::max(max, 1 + calculate(strs, memo, i+1, m-zero_count, n-one_count));
        }
        memo[i][m][n] = max;
        return max;
    }
