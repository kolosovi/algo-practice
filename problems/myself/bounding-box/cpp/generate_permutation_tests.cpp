...
public:
    ...
    void Generate() {
        ...
        int case_number = 1;
        for (auto to_add = points_;
                has_to_add;
                has_to_add = std::next_permutation(to_add.begin(), to_add.end())
                ) {
            bool has_to_remove = true;
            for (auto to_remove = points_;
                    has_to_remove;
                    has_to_remove = std::next_permutation(to_remove.begin(), to_remove.end())
                    ) {
                ...
                for (int i = 0; i < to_remove.size(); i++) {
                    ...
                    case_lines.emplace_back(formatCommand("Rem", {remove_x, remove_y}, box));
                }
...
private:
    ...
    std::string formatCommand(const std::string &tag,
            std::pair<int, int> point,
            std::pair<std::pair<int, int>, std::pair<int, int>> box) {
        std::stringstream command;
        command << tag << " " << formatPoint(point) << " {" << formatPoint(box.first) << "," << formatPoint(box.second) << "}";
        return command.str();
    }
